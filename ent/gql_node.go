...
func (tl *TestList) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     tl.ID,
		Type:   "TestList",
		Fields: make([]*Field, 5),
	...
	if buf, err = json.Marshal(tl.Name); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "string",
	...
	if buf, err = json.Marshal(tl.ValidFrom); err != nil {
		return nil, err
	}
	node.Fields[3] = &Field{
		Type:  "time.Time",
	...
	if buf, err = json.Marshal(tl.ValidTo); err != nil {
		return nil, err
	}
	node.Fields[4] = &Field{
		Type:  "time.Time",
...
func (c *Client) noder(ctx context.Context, table string, id xid.ID) (Noder, error) {
	switch table {
	case testlist.Table:
		query := c.TestList.Query().
			Where(testlist.ID(id))
		query, err := query.CollectFields(ctx, "TestList")
		if err != nil {
			return nil, err
		}
